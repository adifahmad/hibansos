...
router.post('/login', async function(req, res, next){
  try{
    ...
    // use userid column (not id)
    req.session.user = { usersid : users[0].userid, email : users[0].email, username : users[0].username, role: users[0].role }; 
...
router.post('/register/step2', (req, res) => {
  ...
  const { username, name, email, password, repassword, role, organization } = req.body;
  
  if (!username || !name || !email || !password || password !== repassword || !role || !organization) {
    console.log("VALIDASI GAGAL");
    return res.redirect('/register');
  }
  console.log("VALIDASI OK");
  req.session.tempUser = { username, name, email, password, role, organization };
...
router.post('/register', async function(req, res, next) {
  try {
    ...
    const { username, name, email, password, role, organization } = data;
    ...
    await db.query(
        `INSERT INTO users (username, name, email, password, role, organization, latitude, longitude, address)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
        [username, name, email, hash, role, organization, latitude, longitude, address]
      );
...
router.post('/register/verify-otp', async (req, res) => {
  try {
    ...
    const { username, name, email, password, role, organization } = tempUser;
    ...
    if (rows.length === 0) {
      ...
      await db.query(
        `INSERT INTO users (username, name, email, password, role, organization, latitude, longitude, address, email_verified, created_at)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,TRUE,NOW())`,
        [username, name, email, hashPwd, role, organization, lat, lon, address]
      );
    } else {
      await db.query(
